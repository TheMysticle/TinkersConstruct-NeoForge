...
public class ToolModel implements IUnbakedGeometry<ToolModel> {
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker, Function<Material,TextureAtlasSprite> spriteGetter, ModelState modelTransform, ItemOverrides overrides) {
    ...
    if (showTraits) {
      TConstruct.LOG.warn("Using deprecated key 'show_traits' in tool model, use 'constant' in modifier model maps with TraitModel instead");
    }
    for (FirstModifier modifier : firstModifiers) {
      if (modifier.forced) {
        TConstruct.LOG.warn("Using 'forced' in 'first_modifiers' is deprecated in tool model, use 'constant' in modifier model maps instead");
        break;
      }
    }
    ...
    if (modelLocation == null) {
      modelLocation = TConstruct.getResource("tool_model");
    }
