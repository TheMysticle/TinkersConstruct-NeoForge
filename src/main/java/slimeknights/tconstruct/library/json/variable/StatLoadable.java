...
public enum StatLoadable implements Loadable<Stat<?>> {
  ...
  public static Component statName(Stat<?> stat) {
    ...
    } else if (registry.key() == net.minecraft.core.registries.Registries.ENCHANTMENT) {
      if (value instanceof net.minecraft.core.Holder<?> holder && holder.value() instanceof Enchantment ench) {
        name = ench.description();
      } else if (value instanceof Enchantment ench) {
        name = ench.description();
      } else {
