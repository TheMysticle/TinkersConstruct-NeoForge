...
public record ShieldBannerModifierSpriteSource(int cropX, int cropY, int cropWidth, int cropHeight, ResourceLocation destinationPrefix, int offsetX, int offsetY, int outSize) implements SpriteSource {
  public static final ResourceLocation ID = TConstruct.getResource("shield_banner_to_modifier");
  ...
  public void run(ResourceManager manager, Output output) {
    String prefix = "textures/entity/shield/";
    for (Map.Entry<ResourceLocation, Resource> entry : manager.listResources("textures/entity/shield", location -> location.getPath().endsWith(".png")).entrySet()) {
      ResourceLocation input = entry.getKey();
      String path = input.getPath();
      if (!path.startsWith(prefix)) {
        continue;
      }
      String assetPath = path.substring(prefix.length(), path.length() - 4);
