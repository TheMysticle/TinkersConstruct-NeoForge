      ...
      if (recipe.hasEffects()) {
        ...
        if (amount > 0) {
          ...
          for (int shotIndex = 0; shotIndex < shots; shotIndex++) {
            FluidStack shotFluid = fluid.copy();
            shotFluid.setAmount(amount);
            FluidEffectProjectile spit = new FluidEffectProjectile(world, entity, shotFluid, power);
            // apply fins
  ...
  public boolean startInteract(IToolStackView tool, ModifierEntry modifier, Player player, EquipmentSlot slot, TooltipKey keyModifier) {
    if (!tool.isBroken() && keyModifier == TooltipKey.NORMAL) {
      ...
      if (fluid.getAmount() >= modifier.getLevel() && FluidEffectManager.INSTANCE.find(fluid.getFluid()).hasEffects()) {
        ...
        tool.getPersistentData().putInt(modifier.getId().location(), player.tickCount + time);
        return true;
      }
    }
    return false;
